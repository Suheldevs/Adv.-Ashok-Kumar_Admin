...
const SidebarItem = ({ name, icon, active, onClick, collapsed }) => {
  return (
    <div
      className={`flex items-center ${active ? 'bg-slate-600 text-white' : 'text-gray-300 hover:bg-gray-700'} rounded-lg cursor-pointer transition-all duration-200 mb-1 px-3 py-2.5 group`}
      onClick={() => onClick(name)}
    >
      <div className={`${active ? 'text-white' : 'text-gray-400 group-hover:text-white'} mr-3 transition-colors`}>{icon}</div>
      {!collapsed && (
...
const Dashboard = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      {/* Sidebar - Desktop */}
      <div className={`hidden md:block bg-gray-900 transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'w-16' : 'w-64'}`}>
      ...
      {/* Sidebar - Mobile */}
      <div className={`fixed inset-y-0 left-0 z-30 w-64 bg-gray-900 transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:hidden`}>
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="flex items-center justify-between h-16 px-4 border-b border-gray-800">
            <a href="https://www.codecrafter.co.in/" target="_blank" rel="noopener noreferrer">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="hidden md:block text-left">
                    <span className="block text-sm font-medium text-gray-900">{user}</span>
                ...
                {userMenuOpen && (
                  <div className="absolute z-50 right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-gray-200 ring-opacity-5 ">
                    <div className="px-4 py-2 text-sm text-gray-500 border-b">
                      Signed in as <span className="font-medium text-gray-900">{user}</span>
                    </div>
